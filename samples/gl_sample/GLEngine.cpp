...
void GLEngineImpl::Draw(
  ...
  auto fbo = RenderFrame(frameInfo, paths);
...
uint32_t GLEngineImpl::RenderFrame(const RenderFrameInfo &info,
...
void GLEngineImpl::Render(const pxr::UsdImagingGLRenderParams &params,
                          const pxr::SdfPathVector &paths) {
  using namespace pxr;
...
void GLEngineImpl::SetCameraState(const pxr::GfMatrix4d &viewMatrix,
...
void GLEngineImpl::SetRenderViewport(pxr::GfVec4d const &viewport) {
  using namespace pxr;
  TF_VERIFY(_taskController);
  _taskController->SetRenderViewport(viewport);
}

pxr::SdfPath GLEngineImpl::_ComputeControllerPath(
...
void GLEngineImpl::SetColorCorrectionSettings(pxr::TfToken const &id) {
  using namespace pxr;
...
/* static */
bool GLEngineImpl::_UpdateHydraCollection(
    pxr::HdRprimCollection *collection, pxr::SdfPathVector const &roots,
...
void GLEngineImpl::_Execute(const pxr::UsdImagingGLRenderParams &params,
