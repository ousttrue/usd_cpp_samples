...
int main(int argc, char** argv)
{
	if (true)
	{
		...
		//pxr::TfDebug::Enable(pxr::HD_DUMP_GLSLFX_CONFIG);
		//pxr::TfDebug::Enable(pxr::HD_DUMP_SHADER_SOURCE);
		//pxr::TfDebug::Enable(pxr::HD_DUMP_SHADER_BINARY);
		//pxr::TfDebug::Enable((pxr::GLF_DEBUG_TEXTURE_IMAGE_PLUGINS);
	}

	// create a window, GLContext & extensions
	DebugWindow window("hydra test - texturing test", 1280, 720);
	window.Init();
