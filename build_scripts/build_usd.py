...
def RunCMake(context: 'InstallContext', force, extraArgs=None):
...
MATERIALX = Dependency("MaterialX", InstallMaterialX,
...
def InstallUSD(context: 'InstallContext', force: bool, buildArgs):
...
subgroup.add_argument("--prefer-safety-over-speed", dest="safety_first",
                      action="store_true", default=True, help="Enable extra safety checks (which may negatively "
                      "impact performance) against malformed input files "
                      "(default)")
subgroup.add_argument("--prefer-speed-over-safety", dest="safety_first",
                      action="store_false", help="Disable performance-impacting safety checks against "
...
try:
    context = InstallContext(HERE / '../_external/usd', args)
except Exception as e:
    PrintError(str(e))
    sys.exit(1)
...
if PYSIDE in requiredDependencies:
    ...
    pyside2Uic = ["pyside2-uic", "python2-pyside2-uic",
                  "pyside2-uic-2.7", "uic"]
