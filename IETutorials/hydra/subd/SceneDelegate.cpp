...
pxr::HdMeshTopology SceneDelegate::GetMeshTopology(pxr::SdfPath const &id)
{
	...
	if ( id == pxr::SdfPath("/plane"))
	{
		...
		pxr::HdMeshTopology triangleTopology(pxr::PxOsdOpenSubdivTokens->catmark, pxr::HdTokens->rightHanded,
	...
	else if ( id == pxr::SdfPath("/cube") )
	{
		...
		pxr::HdMeshTopology topology(pxr::PxOsdOpenSubdivTokens->catmark, pxr::HdTokens->rightHanded,
											 vertCountsPerFace, verts);
		return topology;
	}
}
